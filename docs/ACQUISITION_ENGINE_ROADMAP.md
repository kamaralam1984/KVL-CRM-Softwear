...
| 17.6b | Attribution model breakdown (surfacing `lib/attribution/models.ts`'s 5 models, built Wave 2 but not yet wired to any UI), richer Landing Pages analytics (beyond Wave 2's traffic-only rollup), Lead Journey timeline drill-down (per-lead chronological view of touchpoints + events + identify + creation) | new tab(s) under `components/crm/sections/acquisition/` | 🔴 |
