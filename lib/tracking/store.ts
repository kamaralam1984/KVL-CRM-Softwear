...
async function getVisitorRow(visitorId: string): Promise<VisitorRow | null> {
...
export async function upsertVisitor(input: {
  ...
  try {
    ...
    const existing = await getVisitorRow(input.visitorId);
    if (existing) {
      await db
        .from("visitors")
        .update({
...
export async function recordSessionEnd(input: {
  sessionId: string;
  exitPage?: string;
...
export async function markVisitorIdentified(visitorId: string): Promise<void> {
  try {
    const db = getServerClient();
    await db.from("visitors").update({ identified: true }).eq("visitor_id", visitorId);
  } catch (err) {
 ...
 */
export async function getVisitorAttribution(visitorId: string): Promise<{ source: string; campaign: string } | null> {
  try {
    const db = getServerClient();
    const { data } = await db
      .from("visitors")
...
export async function recordConsent(input: {
  ...
  try {
    ...
    await db.from("visitors").update({ consent_status: input.status }).eq("visitor_id", input.visitorId);
  } catch (err) {
