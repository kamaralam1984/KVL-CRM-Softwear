// Phase 21 — Unified Conversations. POST /api/sms/inbound — Twilio's SMS
// messaging webhook. Same verification/response shape as
// app/api/whatsapp/inbound/route.ts (see its header comment); the only
// difference is the `From` field has no "whatsapp:" prefix to strip.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const limit = rateLimit(`sms:inbound:${clientIp(req)}`, 60, 60_000);
    if (!limit.allowed) {
    ...
    const { conversationId } = await recordInboundMessage("sms", phone, body, params.MessageSid);

    const resolution = await resolveIdentity({ visitorId: `sms:${phone}`, name: "", email: "", phone }, DEFAULT_SITE_ID).catch(() => null);
    ...
    console.error("[messaging] sms inbound error:", err);
    return twiml();
  }
}
